...
#include "LightSensorSystem.hh"

using namespace custom;

//////////////////////////////////////////////////
void LightSensorSystem::PreUpdate(const ignition::gazebo::UpdateInfo &,
  ...
  _ecm.EachNew<ignition::gazebo::components::CustomSensor,
    ...
    [&](const ignition::gazebo::Entity &_entity,
        ...
        const ignition::gazebo::components::ParentEntity *_parent)->bool
      {
        ...
        if (data.Topic().empty())
        {
          std::string topic = scopedName(_entity, _ecm) + "/light_sensor";
          data.SetTopic(topic);
        }

        ignition::sensors::SensorFactory sensorFactory;
        auto sensor = sensorFactory.CreateSensor<custom::LightSensor>(data);
        if (nullptr == sensor)
        {
          ignerr << "Failed to create light_sensor [" << sensorScopedName << "]"
                 << std::endl;
          return false;
        }
...
//////////////////////////////////////////////////
void LightSensorSystem::PostUpdate(const ignition::gazebo::UpdateInfo &_info,
...
//////////////////////////////////////////////////
void LightSensorSystem::RemoveSensorEntities(
    const ignition::gazebo::EntityComponentManager &_ecm)
{
  _ecm.EachRemoved<ignition::gazebo::components::CustomSensor>(
    [&](const ignition::gazebo::Entity &_entity,
        const ignition::gazebo::components::CustomSensor *)->bool
      {
        ...
        {
          ignerr << "Internal error, missing light_sensor for entity ["
                         << _entity << "]" << std::endl;
        }
        return true;
      });
...
IGNITION_ADD_PLUGIN(LightSensorSystem, ignition::gazebo::System,
  LightSensorSystem::ISystemPreUpdate,
  LightSensorSystem::ISystemPostUpdate
)

IGNITION_ADD_PLUGIN_ALIAS(LightSensorSystem, "custom::LightSensorSystem")